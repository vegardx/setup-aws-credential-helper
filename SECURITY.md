...
This action is intended for trusted code in a Linux, non-container GitHub Actions job. The job—not this helper—is the security boundary. Code running as the same job user may be able to request OIDC tokens, invoke the helper, or read issued temporary credentials. File modes and best-effort cleanup do not defend against malicious same-user code or revoke credentials.

Use least-privilege IAM roles, narrowly scoped OIDC trust conditions, short practical session durations, reviewed and pinned workflow dependencies, and ephemeral runners. Never execute an untrusted pull-request head in a privileged OIDC job. See the README's [security model](README.md#security-model) for the complete operating guidance.
