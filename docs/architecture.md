...
The retained Moto suite runs real production-shaped process boundaries with only the compile-time loopback test helper. It covers AWS SDK JS v3, AWS CLI, pinned Terraform/OpenTofu/provider-v6, distinct `identity`, `state`, `deployment`, and `cloudformation` profiles, S3 backend/provider separation, S3 lifecycle, CloudFormation-backed SQS effects, cross-process cache behavior, a long-lived SDK client crossing synthetic expiration, one Terraform/OpenTofu apply per engine with provider operations before and after synthetic expiration and post-boundary provider/backend refresh counters, native Ubuntu 24.04 x64/arm64, and a same-job Linux container. The production bundle remains unable to use local endpoints.
